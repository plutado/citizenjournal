...
"print")b.$frame[0].contentWindow.print();else if(!v(b,g.command,g.value,g.useCSS,c))return false;w(b)}}
function O(a){a=e(a.target).closest("div");backgroundxPos = a.css('backgroundPosition').split(" ")[0];a.css('background-position', backgroundxPos + ' bottom');}function P(a){a=e(a.target).closest("div");backgroundxPos = a.css('backgroundPosition').split(" ")[0];a.css('background-position', backgroundxPos + ' top');}function ba(a){var b=a.data.popup,c=a.target;if(!(b===p.msg||e(b).hasClass(B))){var d=e.data(b,A),h=e.data(d,x),f=s[h],i=f.command,g,j=this.options.useCSS;if(h=="font")g=c.style.fontFamily.replace(/"/g,"");else if(h=="size"){if(c.tagName=="DIV")c=c.children[0];
